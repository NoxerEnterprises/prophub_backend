...
from app.core.enums import AgentStatus, ChatParticipantRole, ChatType, MediaType, MessageType, PropertyStatus
...
class ChatService:
    ...
    async def start_or_get_private_chat(self, *, current_user: User, payload: ChatCreateRequest) -> tuple[Chat, Message | None]:
        ...
        if not chat:
            ...
            await self.chats.add_participant(
                ChatParticipant(chat_id=chat.id, user_id=current_user.id, role=ChatParticipantRole.MEMBER.value, joined_at=now)
            )
            await self.chats.add_participant(
                ChatParticipant(chat_id=chat.id, user_id=agent.user_id, role=ChatParticipantRole.MEMBER.value, joined_at=now)
            )
        ...
        await self.session.refresh(chat, attribute_names=["participants", "property"])
        if created_message:
        ...
        for chat in chats:
            ...
            items.append(
                ChatListItem(
                    ...
                    property=PropertyChatSummary(id=chat.property.id, title=chat.property.title, state=chat.property.state, community=chat.property.community)
                    if chat.property
                    else None,
    ...
    async def send_text_message(self, *, chat_id: UUID, current_user: User, content: str) -> Message:
        chat = await self.get_chat_for_user(chat_id=chat_id, current_user=current_user)
        message = await self._create_message(chat=chat, sender=current_user, content=content, message_type=MessageType.TEXT)
        await self.session.commit()
        await self.session.refresh(message, attribute_names=["sender"])
        return message

    async def upload_media_message(self, *, chat_id: UUID, current_user: User, file: UploadFile, content: str | None = None) -> Message:
        chat = await self.get_chat_for_user(chat_id=chat_id, current_user=current_user)
