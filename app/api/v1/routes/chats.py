...
async def list_messages(
    ...
    return APIResponse(
        ...
        data=PaginatedResponse(items=[MessageResponse.model_validate(message) for message in messages], meta=PaginationMeta(page=page, limit=limit, total=total)),
    )
...
async def send_text_message(
    ...
    message = await ChatService(session).send_text_message(chat_id=chat_id, current_user=current_user, content=payload.content)
    data = MessageResponse.model_validate(message)
    await websocket_manager.broadcast(chat_id=chat_id, payload={"type": "message.created", "data": data.model_dump(mode="json")})
...
async def send_media_message(
    ...
    message = await ChatService(session).upload_media_message(chat_id=chat_id, current_user=current_user, file=file, content=content)
    data = MessageResponse.model_validate(message)
    await websocket_manager.broadcast(chat_id=chat_id, payload={"type": "message.created", "data": data.model_dump(mode="json")})
    return APIResponse(message="Media message sent", data=data)
